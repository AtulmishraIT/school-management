...
export function Timetable() {
  ...
  const fetchTimetableData = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:5050/api/timetable`, { params })
  ...
  const handleAddEntry = async (entryData) => {
    try {
      ...
      if (editingEntry) {
        const response = await axios.put("http://localhost:5050/api/timetable", entryData)
        console.log("Update response:", response.data)
        if (response.data.error) {
          throw new Error(response.data.error)
        }
      } else {
        await axios.post("http://localhost:5050/api/timetable", entryData)
      }
      fetchTimetableData()
  ...
  const handleDeleteEntry = async (entryId) => {
    if (window.confirm("Are you sure you want to delete this entry?")) {
      try {
        await axios.delete(`http://localhost:5050/api/timetable/${entryId}`)
        fetchTimetableData()
      } catch (error) {

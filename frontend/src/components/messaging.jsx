...
const BulkEmailModal = ({ isOpen, onClose, currentUser }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true)
    try {
      await axios.post("http://localhost:5050/api/email/send-bulk", {
...
export function Messaging() {
    ...
    const fetchData = async () => {
      try {
        const [usersResponse, groupsResponse] = await Promise.all([
          axios.get("http://localhost:5050/api/user/allusers"),
          axios.get(`http://localhost:5050/api/groups/user/${currentUser.id}`),
        ])
  ...
  const fetchConversationHistory = async (otherUserId) => {
    try {
      const response = await axios.get(
        `http://localhost:5050/api/messages/conversation/${currentUser.id}/${otherUserId}`,
      )
      setMessages((prev) => ({
        ...prev,
  ...
  const fetchGroupMessages = async (groupId) => {
    try {
      const response = await axios.get(`http://localhost:5050/api/groups/${groupId}/messages`)
      setGroupMessages((prev) => ({
        ...prev,
  ...
  const handleSendDirectMessage = async () => {
    ...
    try {
      ...
      await axios.post("http://localhost:5050/api/messages/send", {
        senderId: currentUser.id,
        receiverId: selectedChat._id || selectedChat.id,
        message: newMessage.trim(),
      })
  ...
  const handleSendGroupMessage = async () => {
    try {
      const response = await axios.post(`http://localhost:5050/api/groups/${selectedChat._id}/messages`, {
        senderId: currentUser.id,
        message: newMessage.trim(),
        messageType: "text",
      })
  ...
  const handleCreateGroup = async (groupData) => {
    try {
      const response = await axios.post("http://localhost:5050/api/groups", {
        ...groupData,
  ...
  const handleSendEmailAnnouncement = async (emailData) => {
    ...
    try {
      await axios.post(`http://localhost:5050/api/groups/${selectedChat._id}/email-announcement`, {
        senderId: currentUser.id,
        ...emailData,
      })
